...
class TestAnalyzeTechnicalDepth:
    ...
    def test_technical_depth_with_terms(
        self, analyzer_with_domain_terms, text_with_technical_terms
    ):
...
class TestScore:
    ...
    def test_score_high_voice(self, analyzer):
        ...
        analysis = {"first_person": 5, "direct_address": 15, "contractions": 3, "total_words": 100}
    ...
    def test_score_low_voice(self, analyzer):
        ...
        analysis = {"first_person": 0, "direct_address": 15, "contractions": 0, "total_words": 100}
        score, label = analyzer.score(analysis)
    ...
    def test_score_very_low_voice(self, analyzer):
        ...
        analysis = {"first_person": 0, "direct_address": 0, "contractions": 0, "total_words": 100}
        score, label = analyzer.score(analysis)
...
class TestMonotonicIncreasingScoring:
    ...
    def test_calculate_score_monotonic_increasing(self, analyzer):
        ...
        for ratio in ratios:
            metrics = {
                "available": True,
                "voice": {"contractions": int(ratio * 1000), "total_words": 1000},
            }
        ...
        for i in range(len(scores) - 1):
            assert (
                scores[i] <= scores[i + 1]
            ), f"Score decreased: {scores[i]} -> {scores[i + 1]} at ratio {ratios[i]} -> {ratios[i+1]}"
...
class TestIntegration:
    ...
    def test_with_domain_terms_pipeline(
        self, analyzer_with_domain_terms, text_with_technical_terms
    ):
