...
class TestDimensionMetadata:
    ...
    def test_dimension_description(self, dimension):
        ...
        assert any(
            term in desc.lower()
            for term in ["marker", "hedge", "certainty", "epistemic", "pragmatic"]
        )
...
class TestPatternCounts:
    ...
    def test_total_pattern_coverage(self, dimension):
        ...
        assert (
            subjective_certainty == 8
        ), f"Expected 8 subjective certainty, got {subjective_certainty}"
        ...
        assert (
            likelihood_adverbials == 11
        ), f"Expected 11 likelihood adverbials, got {likelihood_adverbials}"
