...
class TestParameterDataclasses:
    ...
    def test_threshold_parameters(self):
        ...
        params = ThresholdParameters(boundaries={
            'excellent_good': 75.0,
            'good_acceptable': 50.0,
            'acceptable_poor': 25.0
        })
...
class TestParameterDeriver:
    ...
    def test_derive_threshold_parameters(self):
        ...
        params = deriver._derive_threshold_parameters(
            human_stats, ai_stats, combined_stats
        )
        ...
        assert (params.boundaries['excellent_good'] >
                params.boundaries['good_acceptable'])
        assert (params.boundaries['good_acceptable'] >
                params.boundaries['acceptable_poor'])
    ...
    def test_derive_all_parameters_subset(self):
        ...
        all_params = deriver.derive_all_parameters(
            analysis,
            dimension_names=['burstiness', 'lexical']
        )
    ...
    def test_roundtrip_preserves_data(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            threshold = ThresholdParameters(boundaries={
                'excellent_good': 85.0,
            ...
            deriver.save_parameters(
                params,
                file_path,
                metadata={'version': '2.5', 'created': '2025-11-24'}
            )
            loaded = ParameterDeriver.load_parameters(file_path)
