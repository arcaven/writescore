...
- [ ] **Task 1: Create CI Workflow** (AC: 1, 2, 3, 4)
  - [ ] Create `.github/workflows/ci.yml`
  - [ ] Configure trigger on push to main
  - [ ] Configure trigger on pull_request
  - [ ] Set up Python matrix: [3.8, 3.10, 3.12]
  - [ ] Add linting step (ruff)
  - [ ] Add unit test step
  - [ ] Add integration test step
  - [ ] Configure test results reporting
  - [ ] Add coverage upload (optional: codecov)

- [ ] **Task 2: Create Release Workflow** (AC: 5, 6, 7)
  - [ ] Create `.github/workflows/release.yml`
  - [ ] Configure trigger on tags matching `v*`
  - [ ] Build Python package (wheel + sdist)
  - [ ] Create GitHub Release
  - [ ] Auto-generate release notes from commits
  - [ ] Attach built artifacts to release
  - [ ] Optional: Publish to PyPI (test.pypi.org initially)

- [ ] **Task 3: Configure Linting** (AC: 3)
  - [ ] Add ruff configuration to pyproject.toml (see Dev Notes for full config):
    ```toml
    [tool.ruff]
    line-length = 100
    target-version = "py38"
    ...
    ```
  - [ ] Run `ruff check src/writescore tests` and fix critical issues
  - [ ] Document any intentionally ignored rules in pyproject.toml comments

- [ ] **Task 4: Verify & Update CHANGELOG** (AC: 8)
  - [ ] Verify CHANGELOG.md already follows Keep a Changelog format (it does)
  - [ ] Add v6.3.0 release section with CI/CD additions:
    ```markdown
    ## [6.3.0] - 2025-11-XX
    ### Added
    ...
    ```
  - [ ] Update [Unreleased] link at bottom of file
  - [ ] Ensure version links at bottom include v6.3.0

- [ ] **Task 5: Consolidate Version Management** (AC: 9)
  - [ ] Verify version is ONLY in pyproject.toml (currently 6.1.0, will bump to 6.3.0)
  - [ ] Bump version in pyproject.toml to 6.3.0
  - [ ] Update `src/writescore/__init__.py` to read version dynamically:
    ...
    ```
  - [ ] Verify `writescore --version` works after `pip install -e .`

- [ ] **Task 6: Update README with Badges** (AC: 10)
  - [ ] Add CI status badge:
    ```markdown
