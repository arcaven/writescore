...
class ScoreNormalizer:
    ...
    def load_statistics(self, stats_path: Path) -> None:
        ...
        try:
            ...
            for dim_name, dim_stats in self.stats.items():
                ...
                if missing:
                    raise ValueError(
                        f"Dimension {dim_name} missing required fields: {missing}"
                    )
    ...
    def normalize_score(self, raw_score: float, dimension_name: str) -> float:
        ...
        if stdev == 0.0:
            logger.warning(
                f"Dimension '{dimension_name}' has stdev=0, using stdev=1.0 fallback"
            )
            stdev = 1.0
    ...
    def compute_dimension_statistics(
        ...
        logger.info(
            f"Computed statistics for {len(computed_stats)} dimensions "
